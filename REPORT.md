...
Out of 30 automated tests, 24 passed and 6 failed. All 6 failures are intentional — they document real bugs discovered in the system during testing.

**Bugs found (6 total):**

| Bug | Requirement violated | Description |
| --- | --- | --- |
| MC-01 | REQ-05 | No overdue warning shown when returning a late book |
| MC-02 | REQ-04 | Borrow limit of 3 books is not enforced (app allows 4+) |
| MC-04 | REQ-07 | Invalid email format (no dot in domain) is accepted when adding a member |
| MC-08 | REQ-04 | 4th borrow attempt succeeds even after confirmation dialog; effective limit is 4, not 3 |
| MC-09 | REQ-08 | Any member can look up and view another member's borrow records |
| MC-10 | REQ-08 | Any member can return a book borrowed by another member without permission check |

**Assessment:** The core library functions (login, search, filter, borrow, return, language switch) work correctly. The main quality concerns are in access control (REQ-08) and business rule enforcement (REQ-04, REQ-05, REQ-07). Specifically, REQ-08 has multiple violations — the system lacks proper authorization checks on member-scoped operations, which is a significant security and data integrity issue.
