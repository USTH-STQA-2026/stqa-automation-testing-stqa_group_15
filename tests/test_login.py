...
def test_login_success(page, test_config):
    ...
    if test_config["display_name"]:
        ...
        assert has_logout, "Login failed: Logout button not found (Không tìm thấy nút Đăng xuất)"
...
def test_login_fail_wrong_password(page, test_config):
    ...
    # [I] Infection: nhập đúng email, sai mật khẩu
    flutter_fill(page, "Email", test_config["email"])
    ...
    # [R✓] Revealability
    sem_text = " ".join(page.locator("flt-semantics").all_text_contents())
    assert "Mật khẩu không đúng" in sem_text, f"Expected error message not found: {sem_text[:200]}"
...
def test_login_fail_empty_fields(page, test_config):
    ...
    # [I] Infection: không nhập gì, click đăng nhập
    flutter_click_button(page, "Đăng nhập")

    # [P] Propagation: chờ thông báo lỗi
    ...
    # [R✓] Revealability
    sem_text = " ".join(page.locator("flt-semantics").all_text_contents())
    assert "Vui lòng nhập email và mật khẩu" in sem_text, f"Expected error message not found: {sem_text[:200]}"
    assert "Đăng nhập" in sem_text, "Should still be on login page"
